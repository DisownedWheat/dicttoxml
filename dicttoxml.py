...
def get_xml_type(val):
    ...
    if isinstance(val, dict):
        return 'dict'
    if isinstance(val, collections.UserList) or isinstance(val, list):
        return 'list'
...
def convert(obj, ids, attr_type, item_func, cdata, parent='root'):
    ...
    if isinstance(obj, collections.UserList) or isinstance(obj, list):
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata):
    ...
    for key, val in obj.items():
        ...
        elif isinstance(val, collections.UserList) or isinstance(val, list):
            if attr_type:
                attr['type'] = get_xml_type(val)
            addline('<%s%s>%s</%s>' % (
                key,
...
def convert_list(items, ids, parent, attr_type, item_func, cdata):
    ...
    for i, item in enumerate(items):
        ...
        elif isinstance(item, collections.UserList) or isinstance(item, list):
            if not attr_type:
                addline('<%s %s>%s</%s>' % (
